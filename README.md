...
- 過去釣果を登録するときは、地名検索、過去地点候補、緯度経度入力で場所を指定可能
...
地名検索はGoogle Maps JavaScript APIのGeocoderを使います。Vercel公開後に動かない場合は、Google Cloud Consoleで利用中のAPIキーに `Geocoding API` または Maps JavaScript API上のGeocoder利用が許可されているか確認してください。
