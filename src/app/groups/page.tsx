  ...
  useEffect(() => {
    getGroupsForUser(userId)
      .then(async (groups) => {
        ...
        setItems(next);
        setMessage(next.length ? "" : "所属しているグループはまだありません。");
      })
    ...
    <>
      ...
      <main className="mx-auto max-w-5xl space-y-5 px-4 py-5">
        <section className="rounded border border-teal-100 bg-white p-4 shadow-soft">
          ...
          <p className="mt-2 text-sm font-bold leading-6 text-slate-600">日常の釣果を仲間内で共有し、ランキング・マップ・分析を見返せます。</p>
          <div className="mt-3 grid gap-2 sm:grid-cols-2">
            ...
            <Link href="/groups/join" className="tap-target rounded border border-water px-4 py-3 text-center font-black text-water">招待コードで参加</Link>
          </div>
        </section>
